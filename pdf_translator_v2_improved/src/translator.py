...
class Translator:
    ...
    def _build_system_prompt(self) -> str:
        """构建系统提示词"""
        source_lang = self._get_lang_name(self.source_lang)
        target_lang = self._get_lang_name(self.target_lang)
        
        example_formula = r"\int_0^\infty e^{-x^2} dx = \frac{\sqrt{\pi}}{2}"
        
        return f"""你是一个专业的学术论文翻译助手。请将{source_lang}文本翻译成{target_lang}。

翻译要求：
1. 保持学术论文的专业性和准确性
2. 保留所有 LaTeX 公式（用 $ 或 $$ 包围的内容）不要翻译，保持原样
